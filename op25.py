...
def gradient_sharpening(image, alpha):
  ...
  mask = np.abs(laplacian)


  sharpened_image = image + alpha * mask * (image - blur)
...
sharpened_image = gradient_sharpening(image.copy(), alpha=1.0)
